...
注册地址:haojiahuo.live/auth/register
...
let check_list = [
				...
				{
			      name: "IKUUU VPN",
				  email: "424337638@qq.com",
				  pwd: "chen7132",
				  login_url: "https://ikuuu.co/auth/login",
                 ...
                 //第三个机场，以此类推。
				{
				  ...
				  email: "424337638@qq.com",
				  pwd: "chen7132",
				  login_url: "https://cwv587.com/auth/login",
				  check_url: "https://cwv587.com/user/checkin"
				},
];
...
!(async () => {
  ...
  console.log("请根据脚本内容修改账号密码！！！！\n")
  
  await check_list.forEach( async(item,index)=>{
	  if (item.email == "" && item.pwd == "") {
		  consol.log(item.name+"邮箱密码为空,跳过执行\n\n");
		  return;
	  }else{
		  await login(item);
	  }
      ...
      async (err, resp, data) => {
        try {
          ...
          if (resp.statusCode == 200) {
            console.log(`${item.name}登录成功`);
            let cookie = resp.headers["set-cookie"];
            console.log(`${item.name}机场获取Cookie成功,正在运行签到...`);
            let checkurl=item.check_url; 
            			
            await check(item.name, checkurl, cookie);
          } else {
            let msg = "登录失败" + data.msg;
            console.log(`${item.name}机场 ${msg}\n\n`);
            await notify.sendNotify(`${$.name}`, `${item.name}机场 ${msg}`);
            return;
          }
        } catch (e) {
          console.log(e);
